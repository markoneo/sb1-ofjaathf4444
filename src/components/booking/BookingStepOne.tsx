...
export default function BookingStepOne({ formData, onChange, onNext }: BookingStepOneProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.priceEstimate && formData.passengers <= 8) {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {isPriceLoading ? (
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="animate-pulse">Calculating your price...</div>
          </div>
        ) : formData.priceEstimate && (
          <div className="bg-blue-50 p-6 rounded-lg space-y-3">
            <h3 className="text-xl font-semibold text-blue-900">Price Estimate</h3>
            {formData.priceEstimate.isCustomQuote ? (
              <div className="space-y-3">
                <p className="text-blue-800">
                  For groups larger than 8 passengers, we'll prepare a custom quote based on your specific needs.
                </p>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-blue-700">Distance</p>
                    <p className="text-lg text-blue-900">{Math.round(formData.priceEstimate.distance)} km</p>
                  </div>
                  <div>
                    <p className="text-sm text-blue-700">Duration</p>
                    <p className="text-lg text-blue-900">{Math.round(formData.priceEstimate.duration / 60)} min</p>
                  </div>
                </div>
                <p className="text-sm text-blue-600">
                  * We'll send a detailed quote to your email within 2 hours.
                </p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-blue-900">€{formData.priceEstimate.price}</p>
                  </div>
                  <div>
                    <p className="text-sm text-blue-700">Distance</p>
                    <p className="text-lg text-blue-900">{Math.round(formData.priceEstimate.distance)} km</p>
